...
items = [
    ...
    ("U-11", "UI", "Distribute-to-WBS dialog on the Cost Mapping workbench", "UNALLOCATED exception rows had no action — a coordinator could see \"0 of 1840 distributed\" but had nowhere to act on it. Added a Distribute button opening a dialog over the existing distributeToWBS action: choose TPL-SINGLE (whole line, one element) or TPL-FLOORS/TPL-ZONES (weighted split across several), pick the target WBS element(s), submit. costMappingSummary's exception payload gained boqId (it only carried boqItemId before) so the dialog can address the right BOQ without an extra round trip", "Done", "Verified live end to end in the browser: distributed item 1.E.1.1.01 (0 of 1840) via TPL-SINGLE onto WBS-0.10 — toast confirmed \"1 line(s) distributed across 1 WBS element(s)\", the WBS Distribution tile moved 0→1 of 5, the exception count dropped 9→8 and the resolved row left the queue. Full regression unaffected: 13 suites / 271 checks green", "The multi-target TPL-FLOORS/TPL-ZONES path is wired and passes the same server-side validation test_distribution.py exercises, but was only exercised via TPL-SINGLE in this browser pass — worth a live click-through with 2+ targets before calling the UI path fully proven. Also found and fixed a real latent bug while building this: `class` is not a valid constructor setting in JS-built controls (only in XML) — silently no-ops with a console warning. Present on this new dialog and on MasterTemplates' pre-existing instantiate dialog; both now use addStyleClass()"),
