...
items = [
    ...
    ("B-06x", "Commercial", "Budget engine", "Generated from the priced build-up by CBS x cost nature (six); refuses unpriced resources whole; approval via BUD-STD; baseline makes the ledger the only door (PATCH on a baselined amount answers 403); zero-sum paired SHIFT with mandatory reason; refreshControl pulls open reservation encumbrance into the control record", "Done", "25 checks; line amount == sum of its ledger entries asserted per line", "Risk Transfer and Variation documents feeding their ledger categories"),
...
opens = [
    ...
    ("Q-09p", "S/4 sandbox tenant - PROGRESS", "Credentials arrived in .env; one careful probe: the API host S4-SANDBOX-api.s4hana.cloud.sap is reachable and speaks OData, but auth is 401 - the supplied user is not a communication user", "Create a communication user (Maintain Communication Users), a communication system, and arrangements starting SAP_COM_0308; put that user in .env; I re-probe on your word. 403 next time = credentials good, arrangement missing; 200 = connector unblocked", "S/4 connector build", "Open", "Before the connector"),
